...
function toProject(row: Row): Project {
  return {
    ...
    tags: typeof row.tags === 'string' ? (() => { try { return JSON.parse(row.tags as string); } catch { return []; } })() : row.tags ?? [],
    technology: typeof row.technology === 'string' ? (() => { try { return JSON.parse(row.technology as string); } catch { return []; } })() : row.technology ?? [],
    favorite: Boolean(row.favorite),
    pinned: Boolean(row.pinned),
