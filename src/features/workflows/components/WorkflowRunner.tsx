      ...
      <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm" onClick={onClose}>
        <div className="w-full max-w-lg bg-theme-surface border border-theme-border rounded-2xl shadow-2xl max-h-[80vh] flex flex-col" onClick={e => e.stopPropagation()}>
          <div className="flex items-center justify-between px-5 py-4 border-b border-theme-border/30">
            <div>
              <h3 className="text-sm font-semibold text-theme-text">{running ? 'Running...' : log?.status === 'completed' ? 'Completed' : log?.status === 'failed' ? 'Failed' : 'Cancelled'}</h3>
          ...
          <div className="flex-1 overflow-y-auto p-5 space-y-2">
            {steps.length === 0 && <div className="flex items-center justify-center py-8"><FaSpinner className="w-5 h-5 text-theme-icon animate-spin" /></div>}
            {steps.map((step, i) => {
              const s = st[step.status] || st.pending;
              return (
                <div key={step.stepId || i} className={`rounded-xl p-3 border border-theme-border/10 ${s.bg} transition-colors`}>
                  ...
                  {step.output && step.status !== 'pending' && <p className="mt-1.5 text-[10px] text-theme-text/50 font-mono pl-6 break-all">{step.output}</p>}
                  {step.error && <p className="mt-1 text-[10px] text-red-400/80 font-mono pl-6 break-all">{step.error}</p>}
                </div>
              );
            })}
          </div>
          <div className="px-5 py-3 border-t border-theme-border/30 flex justify-between items-center">
            <span className="text-xs text-theme-text/40">{steps.filter(s => s.status === 'success').length}/{steps.length} steps succeeded</span>
            {!running && <button onClick={onClose} className="px-4 py-1.5 text-xs font-medium bg-theme-icon text-white rounded-lg hover:bg-theme-icon/90 transition-colors">Close</button>}
          </div>
        </div>
      </div>
    </Portal>
