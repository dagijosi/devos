    ...
    <aside className="w-56 shrink-0 space-y-4">
      ...
      <div className="relative">
        ...
        <input
          ...
          placeholder="Search…"
          className="w-full bg-theme-surface border border-theme-border/20 rounded-xl pl-9 pr-8 py-2.5 text-xs text-theme-text placeholder:text-theme-text/30 outline-none focus:border-theme-icon/50"
        />
        {searchQuery && (
      ...
      <div className="space-y-0.5">
        <NavBtn active={showFavoritesOnly} onClick={() => { onCategoryChange(null); onToggleFavorites(); }} icon={FaStar} label="Favorites" accent="amber" />
        <NavBtn active={showRecentOnly} onClick={() => { onCategoryChange(null); onToggleRecent(); }} icon={FaClock} label="Recent" />
      </div>
      ...
      <div className="border-t border-theme-border/10 pt-3">
        <button
          ...
          className="flex items-center gap-2 px-3 py-1.5 rounded-lg text-xs text-theme-text/50 hover:text-theme-text hover:bg-theme-background/50 w-full transition-colors"
        >
          <FaCog className="w-3 h-3" /> Editor / browser {showPrefs ? '▾' : '▸'}
        </button>
        {showPrefs && (
...
function NavBtn({
  ...
  active: boolean; onClick: () => void; icon: any; label: string; accent?: 'amber';
}) {
  ...
  return (
    <button
      ...
      className={`w-full flex items-center gap-2.5 px-3 py-2 rounded-lg text-xs transition-colors ${
        active ? activeCls : 'text-theme-text/50 hover:text-theme-text hover:bg-theme-background/50'
      }`}
    >
