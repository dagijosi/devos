    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
      <div className="w-full max-w-lg bg-theme-surface border border-theme-border rounded-2xl shadow-2xl max-h-[80vh] flex flex-col">
        <div className="flex items-center justify-between px-5 py-4 border-b border-theme-border/30">
          <div>
            <h3 className="text-sm font-semibold text-theme-text">Run History</h3>
        ...
        <div className="flex-1 overflow-y-auto p-5">
          {loading ? (
            ...
            <div className="text-center py-8">
              <FaClock className="w-8 h-8 text-theme-text/20 mx-auto mb-2" />
              <p className="text-xs text-theme-text/40">No runs yet</p>
            </div>
          ) : (
            <div className="space-y-2">
              {logs.map((log) => {
                const st = statusConfig[log.status] || statusConfig.running;
                return (
                  <div key={log.id} className="bg-theme-background border border-theme-border/20 rounded-xl p-3">
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-xs font-medium text-theme-text">{st.label}</span>
                      </div>
                      ...
                      {log.step_logs?.map((sl, i) => (
                        <div key={sl.stepId || i} className="flex items-center gap-2 text-[10px] text-theme-text/40">
                          <span className={`w-1.5 h-1.5 rounded-full flex-shrink-0 ${
                            sl.status === 'success' ? 'bg-green-400' :
                            sl.status === 'failed' ? 'bg-red-400' :
                            sl.status === 'running' ? 'bg-blue-400' : 'bg-theme-text/20'
                          }`} />
