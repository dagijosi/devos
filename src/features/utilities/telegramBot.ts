...
async function handleCommand(token: string, entry: ProcessedUpdate) {
  ...
  switch (entry.command) {
    case 'start':
      entry.result =
        `👋 *DevOS Bot*\n\n` +
        `I connect Telegram to your DevOS dashboard. Everything works in a single message.\n\n` +
        `*Commands:*\n` +
        `📝 Send text → saves as note\n` +
        ...
        `📋 \`/recent\`\n` +
        `❓ \`/help\`\n\n` +
        `💡 Add \`@ProjectName\` to any command to link it to a project.`;
    ...
    default:
      entry.result =
        ...
        `📋 \`/recent\`\n` +
        `❓ \`/help\`\n\n` +
        `💡 Add \`@ProjectName\` to link to a project.\n` +
        `_Everything works in one message._`;
      await sendMsg(token, chatId, entry.result);
      break;
  }
}
